...
app.UseEndpoints(endpoints =>
{
    ...
    endpoints.Map("employee/profile/{employeid?}", async context =>  // optional param returns null
    {
        ...
        {
            int? employee_name = Convert.ToInt32(context.Request.RouteValues["employeid"]);
            await context.Response.WriteAsync($"In employee: {employee_name}");
        }
        else
        {
            await context.Response.WriteAsync("Employeid is not supplied");
        }
    });
